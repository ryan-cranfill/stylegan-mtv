...
def tf_custom_adaptive_loss(a,b):
    from encoder.adaptive import lossfun
...
class PerceptualModel:
    ...
    def build_perceptual_model(self, generator, discriminator=None):
        ...
        if self.discriminator_loss is not None:
            # print(generated_image_tensor)
            converted_images = tflib.convert_images_from_uint8(generated_image_tensor, nhwc_to_nchw=True)
            print(converted_images)
            discriminator_output = self.discriminator.get_output_for(converted_images, self.stub)
            reduced = tf.math.reduce_mean(discriminator_output)
            self.loss += self.discriminator_loss * reduced
